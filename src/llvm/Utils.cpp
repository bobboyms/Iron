...
namespace iron
{
    ...
    llvm::LoadInst* LLVM::loadVariable(const std::shared_ptr<hlir::Variable>& variable, llvm::Function* function)
    {
        ...
        if (!alloca) {
            throw LLVMException(llvm_utils::formatError("loadVariable", 
                util::format("Variable '{}' not found", varName)));
        }
     ...
     */
    llvm::Value* LLVM::loadStructField(
        ...
        llvm::Function* function)
    {
        ...
        if (!structAlloca) {
            throw LLVMException(llvm_utils::formatError("loadStructField",
                util::format("Struct '{}' not found", structVar->getRealName())));
        }
                                      ...
                                      const std::string& funcName)
    {
        ...
        auto leftVar = operation->getVarLeft();
        auto rightVar = operation->getVarRight();
